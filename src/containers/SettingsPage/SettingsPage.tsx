    ...
    <div>
      <div className={style.headerRow}>
        <div className={style.titleRow}>
          <div className={`${style.pageTitleCommon} ${style.pageTitle}`}>Settings</div>
        </div>
      </div>
        ...
        <div className={style.col1}>
          <div className={`${style.block} ${style.blockInfo}`}>
            <div className={style.blockHeader}>
              <div className={style.blockTitle}>Account details</div>
              <div className={style.blockSubtitle}>Update your account information.</div>
            </div>
            <div className={style.blockContent}>
              ...
              <div className={style.rowBlock}>
                ...
                <div className={style.value}>{timeZone}</div>
              </div>
              <div className={style.rowBlock}>
            ...
            <div className={style.blockHeader}>
              <div className={style.blockHeaderRow}>
                <div className={style.blockHeaderLeft}>
                  <div className={style.blockTitle}>Email</div>
                  <div className={style.blockSubtitle}>(Optional) Email for billing notifications</div>
                </div>
...
const EmailForm: React.FC<any> = (props: any) => {
  ...
  const [errorEmailStep0, setErrorEmailStep0] = useState(
    "Your email is not required, but highly recommended to avoid data loss from insufficient balances"
  );
  ...
  useEffect(() => {
    ...
    if (email && email?.length > 0 && emailVerified) {
      setStep(3);
    }
  }, [email, emailVerified]);
  ...
  const onVerify = (email) => {
    ...
    setErrorEmailStep2(
      "Please confirm your email address. You will receive an activation email. Select the link in the email to verify your address."
    );
  };
  ...
  return (
    <div className={style.wrapperEmailForms} style={step === 1 ? { height: "320px" } : { height: "248px" }}>
      <div
        className={`${style.containerEmailForms}`}
        style={{
          transform: `translateX(-${step * 25}%)`,
          transition: `all ease-in-out ${animation}s`,
        }}
      >
        <div
          className={`${style.emailForm} ${style.emailForm0}`}
          style={step === 0 ? { opacity: "1" } : { opacity: "0" }}
        >
          <div className={style.wrapperInput}>
            <InputText
              onChange={(e) => setEmailValue0(e.target.value)}
              value={emailValue0}
              id={"email"}
              name={"email"}
              tabindex={0}
        ...
        <div
          className={`${style.emailForm} ${style.emailForm1}`}
        ...
        <div
          className={`${style.emailForm} ${style.emailForm2}`}
          style={step === 2 ? { opacity: "1" } : { opacity: "0" }}
        >
          <div className={style.wrapperInput}>
            <InputText
              onChange={(e) => {}}
        ...
        <div
          className={`${style.emailForm} ${style.emailForm3}`}
