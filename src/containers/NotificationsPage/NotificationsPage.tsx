...
import {getNotifications, setNotificationsPagination, setMakeAllReadNotifications} from "../../modules/user/actions";
...
const NotificationsPage: React.FC<any> = () => {
  ...
  const notifications = useSelector((state: TStore)=>{return state.user.notifications});
  const paginationStore = useSelector((state: TStore)=>{return state.user.notificationsPagination});
  ...
  useEffect(()=>{
    if (isFull(query?.id) && query.id && prevQueryId !== query.id){
      ...
      if (note){
        ...
        if (isNew) {
          dispatch(setMakeAllReadNotifications({
            ids: [+query.id],
            pagination: {
  ...
  useEffect(()=>{
    console.log("prevPaginationStore",prevPaginationStore);
    const pathname = history?.location?.pathname;
    const diffPagination = (paginationStore?.page?.toString() !== pagination?.page?.toString() ||
    ...
    if (query?.id){
      if (typeof prevPaginationStore === 'undefined') { //first mount
  ...
  const onClickPage = (page) => {
    dispatch(setNotificationsPagination({
      page: page,
      perPage: paginationStore?.perPage,
  ...
  const onOpenNotification = useCallback((notification)=>{
    const isNew = notification?.readAt === null;
    if (isNew) {
      dispatch(setMakeAllReadNotifications({
        ids: [+notification?.id],
          ...
          <div className={style.col1}>
            <div className={style.block}>
              {notifications?.items && notifications.items.map((item) => {
                return <NotificationsItemElem key={item.id} item={item} isOpen={query?.id && (+query?.id === +item.id)} onOpenNotification={()=>{onOpenNotification(item)}}/>
              })}
            </div>
            <div className={style.pagination}>
              <Pagination hasSelect={true}
                          valueDropdown={{id: +paginationStore?.perPage, name: +paginationStore?.perPage}}
                          count={paginationStore?.pagesCount}
                          onClickPage={onClickPage}
                          activePage={paginationStore?.page}
                          onSelect={onSelect}
                          dropdowns={[{id: 5, name: 5}, {id: 10, name: 10}, {id: 15, name: 15}]}/>
            </div>
          </div>
          <div className={style.col2}>
            <div className={`${style.block} ${style.blockCustom}`}>
                ...
                <div className={style.blockRow}>
                  <div className={style.blockRowLeft}>
                    Total Messages:
                  </div>
                  <div className={style.blockRowRight}>
    ...
    <div className={`${style.elem} ${!readAt ? style.elemNew : ""}`} id={id}>
      ...
      <div className={style.elemTopRow}>
        <div className={style.elemLeft}>
          ...
          {!readAt && <div className={style.elemIsNew}><LabelNotification text={"new"}/></div> }
        </div>
        <div className={style.elemRight} onClick={() => {
          if (!onOpen){
      ...
      <div className={style.elemText}>
        {onOpen ? description : description.slice(0,150) }
      </div>
    </div>
  )
}
