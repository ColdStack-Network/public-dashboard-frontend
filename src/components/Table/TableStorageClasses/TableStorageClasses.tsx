...
const TableStorageClasses: React.FC<IProps > = ({ data }: IProps) => {
  ...
  //console.log("TableStorageClasses", data);
  const resizeHandler = ()=>{
  ...
  const columnNames = useMemo(()=>{if (isFull(data)) {return data.map((el)=>el.name)} else{return []}}, [data]);
      ...
      { width ? <div className={style.scroll} style={{maxWidth: width}}>
        <div className={style.table}>
          <div className={style.tableHeadRow}>
            ...
            {data && data.map((el)=><div className={`${style.tableHeadItem} ${style.tableTdCenter}`}>{el?.name}</div>)}
          </div>
          {itemsFormed && itemsFormed.map((item)=>{
            return <TableRow item={item} columnNames={columnNames}/>
          })}
        </div>
      </div> : ""
      }
...
const TableRow = ({item, columnNames})=>{
  const width = useMemo(()=>{
    if (!isFull(columnNames)){ return "20%"}
    const count = columnNames?.length;
    return `${(100)/(count + 1)}%`;
  }, [columnNames])
  return (
    <div className={style.tableRow} >
      <div className={`${style.tableTd}`} style={{width: width}}>{item.name}</div>
      {columnNames.map((key)=>{
        return <div className={`${style.tableTd} ${style.tableTdCenter}`} style={{width: width}}>{item[key]}</div>
      })}
    </div>
  )
}
