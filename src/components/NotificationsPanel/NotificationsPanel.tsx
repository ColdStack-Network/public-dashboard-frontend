...
export const NotificationsPanel = ({ items, dropRef, close }) => {
  ...
  const onClick = useCallback(
    (id?: number) => {
      close();
      history.push(`/dashboard/notifications${id ? "?id=" + id : ""}`);
    },
    [close, history]
  );
  return (
    <div className={style.dropdownPanel} ref={dropRef}>
      ...
      {isFull(items) ? (
        <div
          className={style.seeAllContainer}
          onClick={() => {
            onClick();
          }}
        >
        ...
        <div className={style.emptyNotifications}>You Don't Have Any Notifications Yet</div>
      )}
    </div>
  );
};
