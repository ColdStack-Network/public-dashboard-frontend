...
import { closeGiftModal, getGift } from '../../../../modules/user/actions';
...
const GiftmModal = ({visible = false, onClose}: IGiftmModal) => {
	...
	const xsmasGift = useSelector((state: TStore) => state.user.gift.xsmasGift);
	...
	return (
		<Modal titleClassName={style.titleClassName}
				...
				return <div>
					{
						giftState === GIFT_STATES.finished ?
							<div>
								<p className={style.presentText}>You've been gifted <span>{xsmasGift?.amount ? toCLS(xsmasGift?.amount) : 0} CLS tokens</span> in your account!</p>
								<div className={style.giftIconWrapper}>
									<BigCLS />
								</div>
								<p className={style.secondPresentText}>It's pure magic: you can only spend them storing files here, but cannot withdraw them.</p>
							</div>
							:
							<div>
								<p className={style.presentText}>Get your presents from ColdStack.</p>
								<div className={style.giftIconWrapper}>
									<div className={`${style.closed} ${isOpened ? style.closedHide : style.closedShowed}`}>
										<Gift />
									</div>
									<div className={`${style.opened} ${isOpened ? style.openedShowed : style.openedHide}`}>
										<div className={style.confeti}><Confeti /></div>
										<div className={style.giftBox}><OpenedGiftBox /></div>
									</div>
								</div>
							</div>
					}
					<div className={style.buttonWrapper}>
						<Button onClickHandler={() => openHandler()} size="gift" color="secondary">{buttonText()}</Button>
					</div>
				</div>
			}}
		</Modal>
