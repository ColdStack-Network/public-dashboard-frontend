...
const ChartComponent: React.FC = () => {
  ...
  function am4themes_myTheme(target) {
    if (target instanceof am4core.ColorSet) {
      target.list = [am4core.color("#6EBADD")];
    }
  }
  ...
  useEffect(() => {
    ...
    function generateChartData2() {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        // some random number
        const europe = Math.round(Math.random() * 10);
        const namerica = Math.round(Math.random() * 10);
        const asia = Math.round(Math.random() * 10) + 20;
    ...
    function updateFields() {
      ...
      document.getElementById("tofield2").value = chart.dateFormatter.format(
    ...
    <div>
      ...
      <div id="controls" style={{ width: "100%", overflow: "hidden" }} className={style.controls}>
        ...
        <div className={style.buttons}>
          <button id="b1m" className={style.chartButton}>
            1m
          </button>
          <button id="b3m" className={style.chartButton}>
            3m
          </button>
          <button id="b6m" className={style.chartButton}>
            6m
          </button>
          <button id="b1y" className={style.chartButton}>
            1y
          </button>
        </div>
      </div>
      ...
      <div style={{ background: "white" }}>
        <div
          ...
          style={
            tabs === 0
              ? { width: "100%", height: "500px", maxWidth: "100%", marginBottom: "40px" }
              : { display: "none" }
          }
        />
        <div
          id="chartdiv"
          style={tabs === 1 ? { width: "100%", height: "500px", maxWidth: "100%" } : { display: "none" }}
        />
      </div>
    </div>
  );
