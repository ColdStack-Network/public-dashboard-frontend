...
import { INewTicket } from "actions/interfaces";

export function* createWithdrawalOldProcess({ payload }: ICreateWithdrawal) {
  try {
    const { fields, onSuccess } = payload;
    const id: INewTicket = yield call(fetchApi, "post", "201", { url: "/tickets", mainUrl: apiUrl, body: fields });
    yield call(fetchApi, "put", "200", { url: `/tickets/${id.id}/status`, mainUrl: apiUrl, body: { status: "close" } });
    onSuccess();
  } catch (err) {
    if (err?.message) {
